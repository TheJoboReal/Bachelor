...
input = int(
    input(
        "Enter 0 to train a new Q-table or 1 to evaluate an existing Q-table or 2 to train a model using a predefined Q-table: "
    )
)
...
class GridWorldEnv(gym.Env):
    ...
    def heatmap_env(self):
        ...
        heatmap_scaled = zoom(
            heatmap,
            (self.size / heatmap.shape[0], self.size / heatmap.shape[1]),
            order=1,
    ...
    def spawn_agents_random(
        self, agents, seed: Optional[int] = None, options: Optional[dict] = None
    ):
        # We need the following line to seed self.np_random
        ...
        for i, agent in enumerate(agents):
            ...
            if i % 2 == 0:
                agent.location = self.np_random.integers(
                    0, self.size - 1, size=2, dtype=int
                )
    ...
    def get_POI_direction(self, agent):
        ...
        distances = [
            np.linalg.norm(np.array(poi) - np.array([agent_x, agent_y]))
    ...
    def remove_POI(self, agent):
        self._agent_poi[agent.agent_id] = [
            poi
            for poi in self._agent_poi[agent.agent_id]
            if not np.array_equal(poi, agent.location)
        ]
    ...
    def step(self, action, agent):
        ...
        agent.location = np.clip(agent.location + direction, 0, self.size - 1)
...
class swarm:
    ...
    def plot_trajectories(
        self, train_env, num_episodes, episodes_to_plot=None, max_agents=5
    ):
...
if input == 0 or input == 2:
    ...
    with open("q_tables/q_table.pkl", "wb") as f:
