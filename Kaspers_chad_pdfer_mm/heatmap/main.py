...
class Environment:
    ...
    def informative_coverage(self, heatmap,sensor_radius = 8, max_length = 500, contour_smoothing=5, contour_threshold=0.00001):
        start_time = time.time()
        ...
        end_time = time.time()
        print(f"Time taken to generate coverage patterns: {end_time - start_time} seconds with {len(result)} coverage-paths")
    ...
    def plot(self, show_basemap = True, show_features=False, show_heatmap = True, show_coverage = False):
        fig, ax = plt.subplots()
        ...
        if show_heatmap:
            ...
            plt.imshow(heatmap.T, extent=extent, origin='lower', cmap=custom_cmap, alpha=0.4)
        if show_coverage:
            sensor_radius = 8
            ...
            for line in lines:
                for i in range(len(line.coords) - 1):
                    ...
                    num_points = int(np.linalg.norm(np.array(end) - np.array(start)) / (sensor_radius/2)) +1
                    for j in range(num_points + 1):
                        x = start[0] + j * (end[0] - start[0]) / num_points
                        y = start[1] + j * (end[1] - start[1]) / num_points
                        circle = plt.Circle((x, y), sensor_radius,color=plt.cm.tab10(3), fill=True, alpha=0.1, edgecolor='none')
                        ax.add_patch(circle)
        ax.set_xlim(self.minx - self.buffer, self.maxx + self.buffer)
        ax.set_ylim(self.miny - self.buffer, self.maxy + self.buffer)
        # plt.axis("equal")
    ...
    def interactive_plot(self,heatmap, use_sliders=True, show_basemap = True, show_features=False, export=False, show_heatmap = True):
        ...
        if show_heatmap:
            ...
            extent = [self.xedges[0], self.xedges[-1], self.yedges[0], self.yedges[-1]]
            heatmap[heatmap < 0.00001] = 0
            heatmap_img = plt.imshow(heatmap.T, extent=extent, origin='lower', cmap=custom_cmap, alpha=alpha if alpha is not None else 1)
        ...
        if use_sliders:
            ...
            for key in self.heatmaps.keys():
                ...
                sensor_slider = Slider(ax_sensor_slider, 'Sensor Width', 1.0, 20.0, valinit=8, valstep=0.5)
            # Save button
            ...
            def update(val):
                ...
                lines = self.informative_coverage(combined_heatmap,sensor_radius=sensor_slider.val)
                coverage = GeoMultiTrajectory(lines, crs="EPSG:2197")
                coverage.plot(ax, color=plt.cm.tab10(4),add_points=False, alpha=1)
                plt.ion()
...
env.interactive_plot(heatmap, use_sliders=True, show_basemap=True, show_features=False, show_heatmap=True, export=True)
# heatmap = env.get_combined_heatmap(sigma_features, alpha_features)
